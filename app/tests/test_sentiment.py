...
def test_compute_sentiment():
    ...
    # Neutral check
    assert abs(compute_sentiment(
        "The blockchain processes transactions.")) < 0.05
    ...
    assert (
        abs(compute_sentiment("I don't know anything about crypto.")) < 0.05)
    ...
    assert (
        compute_sentiment(
            "We have been manipulate by Elon Musk, it's a scam."
        )
        < 0
    )
    assert compute_sentiment("This project is absolute garbage!") < 0
    assert (
        compute_sentiment("The devs messed up everything terrible update.")
        < 0
    )
    ...
    assert (
        compute_sentiment(
            "My portfolio have recently blew up with profit."
        )
        > 0
    )
