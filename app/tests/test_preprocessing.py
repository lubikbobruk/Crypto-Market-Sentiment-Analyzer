from app.src.utils import (
    ...
    combine_csv)
from app.src.text_preprocessing import soft_clean, hard_clean
import pandas as pd
...
def test_remove_urls():
    assert remove_urls(
        "Check this https://binance.com now") == "Check this  now"
    assert remove_urls(
        "a http://somethin.com b https://test.org c") == "a  b  c"
...
def test_remove_non_ascii():
    ...
    assert (
        remove_non_ascii("Holy Crêpes! BTC to the moon🚀")
        == "Holy Crpes! BTC to the moon")
