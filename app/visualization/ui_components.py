...
def show_config_summary():
    ...
    with c1:
        ...
        st.markdown(f"<h3 style='margin-top:-10px'>{st.session_state['cfg_platform'].capitalize()}</h3>",
                    unsafe_allow_html=True)

    with c2:
        st.markdown("**🪙 Coin**")
        st.markdown(f"<h3 style='margin-top:-10px'>{st.session_state['cfg_coin']}</h3>",
                    unsafe_allow_html=True)

    with c3:
        st.markdown("**📆 Period**")
        st.markdown(f"<h3 style='margin-top:-10px'>{st.session_state['cfg_period']}</h3>",
                    unsafe_allow_html=True)
...
def select_platform():
    ...
    label = st.radio("Platform", PLATFORM_LABELS, horizontal=True,
...
def select_coin():
    ...
    if choice == "Custom":
        val = st.text_input("Enter custom coin:", value=default_custom).upper().strip()
        return val, bool(val)
...
def select_period():
    ...
    label = st.radio("Time window", PERIOD_LABELS, horizontal=True,
...
def apply_config_button(platform, coin, period, enabled=True):
    st.markdown("---")
    if st.button("✅ Apply configuration", disabled=not enabled, use_container_width=True):
