...
from app.collectors.telegram_collector import telegram_collect_async, collect_telegram
...
def run_telegram(keyword, period):
    ...
    try:
        ...
        msgs = loop.run_until_complete(tg_collect(keyword, chans, limit, period, creds))
...
def run_preprocessing(csv_path, platform):
    ...
    soft, _ = preprocess_csv(csv_path,PROCESSED_DIR,platform)
    return soft

def run_sentiment(soft_csv_path, platform):
    return analyze_sentiment(soft_csv_path,PROCESSED_DIR,platform)